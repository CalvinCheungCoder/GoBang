...
- (id)init {
    ...
    self = [super init];
    if (self) {
        self.p = [[DHPoint alloc] initPointWith:-1 y:-1];
        self.count = 0;
    }
...
- (id)initWithPoint:(DHPoint *)point count:(int)ncount {
...
- (BOOL)isStepEmergent:(DHPoint *)pp Num:(int)num type:(OccupyType)xType {
    ...
    DHPoint* check = [[DHPoint alloc] initPointWith:pp.x y:pp.y];
    if (![self checkPoint:check]) {
        return FALSE;
    } else {
        
        DHPoint *test = [[DHPoint alloc] initPointWith:check.x y:check.y]; // 开始进行寻找的点
        DHPoint *testR = [[DHPoint alloc] initPointWith:check.x y:check.y]; // 遍历点的左侧的点
        DHPoint *testL = [[DHPoint alloc] initPointWith:check.x y:check.y]; // 遍历点的右侧的点
        
        testR = [[DHPoint alloc] initPointWith:check.x + 1 y:check.y]; // 开始遍历点的正右侧
        testL = [[DHPoint alloc] initPointWith:check.x - 1 y:check.y]; // 开始遍历的点的正左侧
        
        for(int hi = 1; (test.x+hi < kBoardSize) && ([self.curBoard[(int)testR.x][(int)testR.y] integerValue] == xType); hi++) //在相应点向右侧进行遍历，寻找最长的连续距离
            testR = [[DHPoint alloc] initPointWith:testR.x + 1 y:testR.y];
        for(int hii = 1; (test.x-hii >= 0) && ([self.curBoard[(int)testL.x][(int)testL.y] integerValue] == xType); hii++) //在相应点向左侧进行遍历，寻找最长的连续距离
            testL = [[DHPoint alloc] initPointWith:testR.x - 1 y:testR.y];
        ...
        testR = [[DHPoint alloc] initPointWith:check.x y:check.y + 1]; // 开始遍历点的正下方
        testL = [[DHPoint alloc] initPointWith:check.x y:check.y - 1]; // 开始遍历的点的正上方
...
- (DHPoint *)nextStep:(OccupyType)xType num:(int)num thre:(int)threshold x:(int)startX y:(int)startY {
...
- (DHPoint *)horizontal:(DHPoint *)pp type:(OccupyType)xType num:(int)num thre:(int)threshold {
    ...
    int count = 0;
    DHPoint *solution = [[DHPoint alloc] init];
...
- (DHPoint *)vertical:(DHPoint *)pp type:(OccupyType)xType num:(int)num thre:(int)threshold {
...
- (DHPoint *)rightDown:(DHPoint *)pp type:(OccupyType)xType num:(int)num thre:(int)threshold {
...
- (DHPoint *)leftDown:(DHPoint *)pp type:(OccupyType)xType num:(int)num thre:(int)threshold {
...
- (DHPoint *)getNextPoint:(DHPoint *)pp {
    
    DHPoint *result = [[DHPoint alloc] init];
    result.x = pp.x;
    result.y = pp.y;
...
+ (DHPoint *)geablog:(NSMutableArray *)board type:(OccupyType)type {
    ...
    DHPoint *calibur = [[self class] SeraphTheGreat:board type:type];
    return calibur;
}

+ (DHPoint *)SeraphTheGreat:(NSMutableArray *)board type:(OccupyType)myType {
    
    DHOmni *omniknight = [[DHOmni alloc] init];
    if(myType == OccupyTypeUser)
        
        omniknight = [[DHOmni alloc] initWithArr:board opp:OccupyTypeAI my:OccupyTypeUser];
    else
        
        omniknight = [[DHOmni alloc] initWithArr:board opp:OccupyTypeUser my:OccupyTypeAI];
    DHPoint *calibur;
    ...
    //  如果什么都没有则返回不可用的点
    DHPoint *sad = [[DHPoint alloc] initPointWith:(kBoardSize+1)/2 y:(kBoardSize+1)/2];
    return sad;
}
